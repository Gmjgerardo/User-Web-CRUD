<div class="row">
    ...
    if (isset($dataToView["data"]) && count($dataToView["data"]) > 0) { ?>
        <table class="table">
            ...
            <tbody>
                <?php
                foreach ($dataToView["data"] as $userData) {
                    [$id, $names, $lastName, $secondLastName, $user, $rol, $fecha_creacion] = $userData;
                ?>
                    <tr>
                        <th scope="row"><?= $id ?></th>
                        <td><?= $names ?></td>
                        ...
                        <?php if ($_SESSION["login"]["rol"] >= 1) { ?>
                            <td>
                                <!-- Botón Editar -->
                                <a class="btn bg-warning-subtle btn-sm w-25" href="<?= ROOT . "?controller=user&action=edit&id=" . $id ?>">
                                ...
                                <!-- Botón Eliminar -->
                                <a class="btn bg-danger-subtle btn-sm w-25" href="<?= ROOT . "?controller=user&action=delete&id=" . $id ?>">
    ...
    <?php
        ...
        <div class="alert alert-info">
            No hay usuarios registrados en el sitema
        </div>
    <?php } ?>
